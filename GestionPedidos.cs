        ...
        public void AgregarPedido()
        {
            try
            {
                while (true)
                {
                    ...
                    Negocio[] negociosPredefinidos = GestionDeNegocios.ObtenerNegociosPredefinidos();

                    Console.WriteLine("Seleccione un negocio :");
                    for (int i = 0; i < negociosPredefinidos.Length; i++)
                    {
                        Console.WriteLine($"\n {i + 1}.{negociosPredefinidos[i].Nombre} - {negociosPredefinidos[i].Ubicacion}, {negociosPredefinidos[i].Telefono}, {negociosPredefinidos[i].Email}\n");
                    }
                    ...
                    if (opcionNegocio >= 1 && opcionNegocio <= negociosPredefinidos.Length)
                    {
                        ...
                        while (true)
                        {
                            ...
                            Producto[] productosCategoria = GestionProductos.ObtenerProductosPredefinidos();
                            productosCategoria = Array.FindAll(productosCategoria, p =>
                            {
            ...
            while (true)
            {
                ...
                if (opcionProducto >= 1 && opcionProducto <= productosCategoria.Length)
                {
                    ...
                    if (int.TryParse(Console.ReadLine(), out int cantidad))
                    {
                        ...
                        {
                            Console.WriteLine($"No hay suficiente stock. Cantidad disponible: {productoSeleccionado.CantidadEnStock}. Presione cualquier tecla para continuar...");
                            Console.ReadKey();
                        }
                    }
                    else
